...
const checks = [
  ...
  ['Payment-proof admin view uses short-lived signed URLs and does not grant access automatically', /createSignedUrl/.test(files.paymentSubmissionAdmin) && /10 \* 60/.test(files.paymentSubmissionAdmin) && /does not grant Premium automatically/.test(files.paymentSubmissionAdmin) && /reviewSubmission/.test(files.paymentSubmissionAdmin)],
